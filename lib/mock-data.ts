...
export interface DoctorProfile {
  ...
  bio: string
  imageUrl?: string
  tagline?: string            // short, doctor-authored personality one-liner shown near the name
  laymanDescription?: string  // the doctor's own plain-language description of what they do (distinct from the generic subspeciality descriptor)
  interests?: string[]        // hobbies / personal interests ("has a life")
  achievements?: string[]     // recent achievements / highlights
  hashtags?: string[]         // display tags, e.g. ['#EpilepsyCare', '#ParentFriendly'] (store WITHOUT requiring the # — include the # in the string for simplicity)
  catchMeAt?: string          // non-contact location line, e.g. 'Aster CMI, Bangalore' (informational only; NOT a contact method)
...
export const SEED_DOCTORS: DoctorProfile[] = [
  // --- Pediatric Cardiology ---
  { id: 'dr-001', name: 'Dr. Priya Sharma', subspeciality: 'Pediatric Cardiology', yearsExperience: 12, pgCollege: 'AIIMS Delhi', workingHospital: 'Narayana Health', fee: 800, rating: 4.8, consultationCount: 245, languages: ['English', 'Hindi', 'Kannada'], city: 'Bangalore', bio: 'Specializes in congenital heart defects and pediatric arrhythmias. Known for her compassionate approach with anxious parents and thorough echocardiogram interpretations.', tagline: 'A tiny heart deserves a calm, unhurried listener.', laymanDescription: 'I look after the hearts of babies and children — from murmurs picked up at a checkup to holes and rhythm problems. Most of the time my job is to reassure you, and to know exactly when something truly needs attention.', catchMeAt: 'Narayana Health, Bangalore', availability: [{ day: 'Monday', time: '09:00 AM' }, { day: 'Monday', time: '09:30 AM' }, { day: 'Wednesday', time: '02:00 PM' }, { day: 'Friday', time: '11:00 AM' }] },
  ...
  // --- Pediatric Neurology ---
  { id: 'dr-003', name: 'Dr. Rajesh Kumar', subspeciality: 'Pediatric Neurology', yearsExperience: 15, pgCollege: 'CMC Vellore', workingHospital: 'Manipal Hospital', fee: 1000, rating: 4.9, consultationCount: 380, languages: ['English', 'Hindi', 'Tamil'], city: 'Chennai', bio: 'Expert in childhood epilepsy and neurodevelopmental disorders. Pioneered video-EEG telemonitoring protocols for remote diagnosis of seizure disorders.', tagline: 'Fits can be frightening to watch — my job is to make them predictable and manageable.', laymanDescription: 'I help children whose brain sends the occasional wrong signal — seizures, staring spells, headaches, or delays in reaching milestones. A lot of what I do is figuring out the pattern, often from a phone video you send me, so we can treat it right without endless hospital visits.', interests: ['Chess (still chasing that elusive rating)', 'Long-distance running and the occasional half-marathon', 'Tinkering with home gadgets and wearable sensors', 'Reading up on the science of sleep'], achievements: ['Set up a video-EEG telemonitoring program that lets rural families get seizures diagnosed without travelling', 'Co-authored a national guideline chapter on childhood epilepsy follow-up'], hashtags: ['#EpilepsyCare', '#TeleNeurology', '#ParentFriendly', '#SeizureSmart'], catchMeAt: 'Manipal Hospital, Chennai', availability: [{ day: 'Tuesday', time: '10:00 AM' }, { day: 'Thursday', time: '03:00 PM' }, { day: 'Saturday', time: '09:00 AM' }] },
  ...
  // --- Pediatric Pulmonology ---
  { id: 'dr-005', name: 'Dr. Anitha Reddy', subspeciality: 'Pediatric Pulmonology', yearsExperience: 10, pgCollege: 'NIMHANS Bangalore', workingHospital: 'Apollo Hospital', fee: 750, rating: 4.7, consultationCount: 190, languages: ['English', 'Telugu', 'Kannada'], city: 'Hyderabad', bio: 'Specializes in pediatric asthma management and chronic lung disease. Advocates for inhaler technique education and long-term action plans for wheezing children.', tagline: 'I spend my days on breathing — and my evenings on it too, one clarinet note at a time.', laymanDescription: 'I look after little lungs — the child who is always coughing, wheezing, breathless while playing, or snoring through the night. Most kids with asthma can run, swim and sleep just fine once we get the plan and the inhaler technique right.', interests: ['Playing the clarinet in a community wind ensemble', 'Choir singing (great for lung capacity, I promise)', 'Trekking in the Western Ghats', 'Growing chillies on her balcony'], achievements: ['Built a school-based asthma action-plan program adopted across several Hyderabad schools', 'Ran a free monsoon-season wheeze clinic for underserved neighbourhoods'], hashtags: ['#AsthmaCare', '#BreatheEasy', '#InhalerTechnique', '#LungsAndMusic'], catchMeAt: 'Apollo Hospital, Hyderabad', availability: [{ day: 'Monday', time: '04:00 PM' }, { day: 'Wednesday', time: '09:00 AM' }, { day: 'Friday', time: '02:00 PM' }] },
  ...
  { id: 'dr-010', name: 'Dr. Deepa Krishnan', subspeciality: 'Pediatric Nephrology', yearsExperience: 9, pgCollege: 'CMC Vellore', workingHospital: 'Apollo Hospital', fee: 800, rating: 4.7, consultationCount: 160, languages: ['English', 'Malayalam', 'Tamil'], city: 'Chennai', bio: 'Focused on glomerular diseases and dialysis care in children. Passionate about educating families on fluid and diet management for kidney health.', imageUrl: '/images/doctors/dr-010.jpg', tagline: 'Kidneys sound scary. My job is to make them make sense — for you and your child.', laymanDescription: 'I look after children with kidney and urine problems — repeated infections, swelling, blood or protein in the urine, bedwetting that just won\'t stop, or high blood pressure. A big part of my work is turning worrying words into a simple plan you can actually follow at home.', interests: ['Bharatanatyam — I trained for years and still perform occasionally', 'Coaxing stubborn roses to bloom in my terrace garden', 'Devouring mystery novels, the twistier the better', 'Weekend baking experiments with my niece'], achievements: ['Published a peer-reviewed paper on early screening for childhood nephrotic syndrome', 'Led a free community kidney-health screening drive across three Chennai schools'], hashtags: ['#KidneyCare', '#GentleWithKids', '#NephroNerd', '#LessScaryMedicine'], catchMeAt: 'Apollo Hospital, Chennai', availability: [{ day: 'Monday', time: '11:00 AM' }, { day: 'Wednesday', time: '02:00 PM' }, { day: 'Friday', time: '10:00 AM' }] },
  // --- Pediatric Endocrinology ---
  { id: 'dr-011', name: 'Dr. Meera Nair', subspeciality: 'Pediatric Endocrinology', yearsExperience: 14, pgCollege: 'JIPMER Puducherry', workingHospital: 'Aster CMI', fee: 850, rating: 4.8, consultationCount: 310, languages: ['English', 'Malayalam', 'Tamil'], city: 'Bangalore', bio: 'Expert in Type 1 diabetes management, growth disorders, and thyroid conditions in children. Uses continuous glucose monitoring data for remote dose adjustments.', tagline: 'Diabetes doesn\'t get to run your child\'s childhood — we manage it together.', laymanDescription: 'I help children whose hormones need a little steering — diabetes, growth that seems too slow or too fast, early or delayed puberty, and thyroid issues. With today\'s glucose sensors, I can often fine-tune things from your child\'s data without you leaving home.', interests: ['Building little home-automation gadgets with a Raspberry Pi', 'Data-viz doodling — she genuinely enjoys a good glucose graph', 'Filter coffee tasting and roasting', 'Casual badminton on weekends'], achievements: ['Started a peer-support group connecting families of children living with Type 1 diabetes', 'Helped roll out remote CGM-based dose adjustment for kids in smaller towns'], hashtags: ['#Type1Together', '#GrowthMatters', '#CGMFriendly', '#HormonesExplained'], catchMeAt: 'Aster CMI, Bangalore', availability: [{ day: 'Monday', time: '10:00 AM' }, { day: 'Wednesday', time: '11:00 AM' }, { day: 'Friday', time: '09:00 AM' }] },
  ...
  // --- Developmental Pediatrics ---
  { id: 'dr-025', name: 'Dr. Kavitha Mohan', subspeciality: 'Developmental Pediatrics', yearsExperience: 12, pgCollege: 'NIMHANS Bangalore', workingHospital: 'Manipal Hospital', fee: 900, rating: 4.8, consultationCount: 260, languages: ['English', 'Kannada', 'Hindi'], city: 'Bangalore', bio: 'Expert in autism spectrum disorders, ADHD, and learning disabilities. Designs individualized therapy plans combining OT, speech, and behavioral interventions.', tagline: 'Every child develops on their own timeline — my job is to understand yours.', laymanDescription: 'I work with children who reach milestones differently — late talking, trouble focusing, difficulty at school, or challenges with social interaction. Together we figure out what helps your specific child thrive, at home and in the classroom.', catchMeAt: 'Manipal Hospital, Bangalore', availability: [{ day: 'Monday', time: '10:00 AM' }, { day: 'Wednesday', time: '10:00 AM' }, { day: 'Friday', time: '02:00 PM' }] },
...
const STORAGE_KEY = 'pedispectra-mock-data-initialized-v3'
